...
from eagle.model.testModules import *
...
#-------------------------------------------------------------------------------------------
    ...
    # print("\nDRAFT TOKEN: {}\n".format(draft_tokens))
    # temp_tokenizer = AutoTokenizer.from_pretrained("/home/he/Desktop/Model/vicuna-7b-v1.3")
    # print("\nDRAFT TOKEN IN WORD: {}\n".format(temp_tokenizer.decode(draft_tokens[0].tolist())))
    return draft_tokens, retrieve_indices,tree_mask,tree_position_ids, orig, hidden_states, token


def initialize_tree_base(input_ids, model, past_key_values, logits_processor):
    # base model generate the first token
    outputs, orig, hidden_states = model(
        input_ids, past_key_values=past_key_values, output_orig=True
    )
    ...
    if model.use_eagle3:
        ...
        if outputs["hidden_states"][0].device != ea_device:
            outputs["hidden_states"] = [x.to(ea_device) for x in outputs["hidden_states"]]
        hidden_states=torch.cat(outputs["hidden_states"],dim=-1)
    return hidden_states, input_ids, orig, token


def initialize_tree_draft(hidden_states, input_ids, model, logits_processor):
...
def update_inference_inputs(
    ...
    # size_mb = get_tgt_size_in_mb(tgt, verbose=True)
    # send_tensor_via_ip(tgt)
    ...
    # testInputIds=torch.cat((input_ids, token), dim=1)
    # inputss = {
    #     "testInputIds": testInputIds,
    #     "accept_hidden_state_new": accept_hidden_state_new,
    #     "logits_processor": logits_processor
    # }
    # input_size = get_total_transfer(inputss)
    # print(f"update inference inputs传输量: {input_size/1024:.2f} KB")
    # send_tensor_via_ip(testInputIds)
    # send_tensor_via_ip(accept_hidden_state_new)
    # draft model start to work
...
@torch.no_grad()
def update_inference_inputs_base(
        input_ids,
        candidates,
        best_candidate,
        accept_length,
    ...
    if logits_processor is not None:
        ...
        token = token[None, None]
    # hidden_state = torch.cat((hidden_state, accept_hidden_state_new), dim=1)
    InputIds=torch.cat((input_ids, token), dim=1)

    return accept_hidden_state_new, InputIds, input_ids, token


@torch.no_grad()
def update_inference_inputs_draft(model, accept_hidden_state_new, InputIds, logits_processor, accept_length, new_token):
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.ea_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=InputIds,
                                              head=model.base_model.lm_head,logits_processor=logits_processor)
    ...
    return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, new_token
