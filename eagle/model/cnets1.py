...
def load_embeddingLayer(model_dir, weight_dir):
    ...
    new_weight = torch.nn.Parameter(
        embedding_weights["weight"].to(torch.float16).to("cpu")
    )
    ...
    print("成功加载嵌入层！")
    print("最终权重类型:", tryembed.weight.dtype) 
    print("\n\nAnother Embedd:\n",tryembed.weight.data)
    return tryembed.weight.data
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        if load_emb:
            ...
            # self.embed_tokens.weight.data = load_embeddingLayer(model_dir="lmsys/vicuna-7b-v1.3", weight_dir="/home/he/Desktop/Model/embeddingLayer/vicuna7b_embeddings.bin")
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        depth = self.depth
        top_k = self.top_k
        print(hidden_states.dtype, input_ids.dtype)
        ...
        for i in range(depth):
            ...
            end_evnet.record()
            torch.cuda.synchronize()
            elapsed_ms = start_event.elapsed_time(end_evnet)
            print("Time for draft model taken: ", elapsed_ms)
            print(out_hidden.dtype, out_hidden.shape)
