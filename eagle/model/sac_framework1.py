...
class Actor_1d(nn.Module):
    ...
    def sample(self, state):
        ...
        if torch.isnan(state).any():
            ...
            state = torch.zeros_like(state)  # emergency recovery
        
        mean, log_std = self.forward(state)
        
        # Stricter output checks
        if torch.isnan(mean).any() or torch.isinf(mean).any():
            print("Actor output abnormal! Forcing recovery...")
            mean = torch.zeros_like(mean)
            log_std = torch.zeros_like(log_std) - 1  # corresponds to std≈0.37
        
        std = log_std.exp() + 1e-6  # ensure std > 0
        ...
        try:
            ...
            x_t = torch.rand_like(mean)  # fallback to random sampling
        
        y_t = torch.tanh(x_t)
        action = y_t * self.action_scale + self.action_bias
        
        # Log-prob calculation protection
        log_prob = normal.log_prob(x_t)
        log_prob -= torch.log(self.action_scale * (1 - y_t.pow(2)) + 1e-6)
        log_prob = torch.clamp(log_prob.sum(1, keepdim=True), -100, 100)
        
        return torch.clamp(action, 0, 1), log_prob
...
class SpeculativeInferenceEnv:
    def __init__(self, 
        ...
        """Speculative decoding environment

        Parameters:
        total_power: total power of edge device (W)
        max_depth: maximum depth
        history_size: history length for accept length
        """
        ...
        # state dimension: [current accept length, current accept ratio, mean historical accept length, mean historical accept ratio, iteration time, H]
        self.state_dim = 6
    ...
    def calculate_iteration_time(self):
        ...
        """
        print("Comp power:", self.comp_power, "Comm power:", self.comm_power)
    ...
    def step(self, action):
        ...
        Args:
            action: [depth_ratio (0-1), comm_power_ratio (0-1)]
...
class SACAgent:
    def __init__(self, state_dim, action_dim, device, 
        ...
        self.target_entropy = -np.log(1/action_dim) * 1.3  # heuristic value
        ...
        self.replay_actions = np.zeros(buffer_size, dtype=np.int32)
    ...
    def update(self, batch_size):
        ...
        with torch.no_grad():
            ...
            # print("Next actions shape:", next_actions.shape)
            next_actions_onehot = F.one_hot(next_actions - 1, num_classes=self.action_dim).float()
            # print("Next actions onehot shape:", next_actions_onehot.shape)
            q1_next, q2_next = self.target_critic(next_states, next_actions_onehot)
        ...
        current_actions_onehot = F.one_hot(actions - 1, num_classes=self.action_dim).float()
        # print("Current actions onehot shape:", current_actions_onehot.shape)
        q1, q2 = self.critic(states, current_actions_onehot)
        ...
        new_actions_onehot = F.one_hot(new_actions - 1, num_classes=self.action_dim).float()
        log_probs = torch.clamp(log_probs, -100, 100)
        with torch.no_grad():
            q1_new, q2_new = self.critic(states, new_actions_onehot)
